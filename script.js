...
async function getName(latitude, longitude) {
  var access_key = "4e32039a6ab6354bfb1ec3ffeeddd974";
  var url =
...
async function fetchData(location) {
  var url =
    `https://api.weatherapi.com/v1/current.json?key=8027325e669e48febba90326242501&q=` +
    location +
    `&aqi=yes`;
  var res = await fetch(url);
  var data = await res.json();
...
function setValues(data) {
  ...
  setter.innerHTML = data.current.temp_c + `&#176` + "C";
  ...
  setter.innerHTML = `Feels like ` + data.current.feelslike_c + `&#176` + "C";
  ...
  setter.innerHTML = data.current.humidity / 10 + `%`;
